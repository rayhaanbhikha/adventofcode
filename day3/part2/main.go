...
func main() {
	...
	fmt.Println(getCoordinates("R75", origin))
	j1 := Journey{currentCoord: origin}
	j2 := Journey{currentCoord: origin}
	// p1 := strings.Split("R75,D30,R83,U83,L12,D49,R71,U7,L72", ",")
	// p2 := strings.Split("U62,R66,U55,R34,D71,R55,D58,R83", ",")
	...
	grid := make(map[Coordinates]GridPoint)

	for _, p := range p1 {
		newCoordinates := getCoordinates(p, j1.currentCoord)
		j1.currentCoord = newCoordinates[len(newCoordinates)-1]

		for _, coord := range newCoordinates {
			j1.stepsTaken++
			if _, exists := grid[coord]; !exists {
				grid[coord] = GridPoint{intersect: false, steps: j1.stepsTaken, pathBelongs: "j1"}
			}
		}
	}
	...
	for _, p := range p2 {
		...
		j2.currentCoord = newCoordinates[len(newCoordinates)-1]

		for _, coord := range newCoordinates {
			j2.stepsTaken++
			if v, exists := grid[coord]; exists && v.pathBelongs == "j1" {
				grid[coord] = GridPoint{intersect: true, steps: v.steps + j2.stepsTaken}
			} else if _, exists := grid[coord]; !exists {
				grid[coord] = GridPoint{intersect: false, steps: j2.stepsTaken}
			}
		}
	}
	...
	for k, v := range grid {
		if v.intersect {
			fmt.Println(k, v.steps)
			if minSteps == 0 || minSteps > v.steps {
				minSteps = v.steps
			}
		}
	}
