...
func main() {
	...
	for k, v := range grid {
		if v == "intersection" {
			if minDistance == origin || manHattanDistance(minDistance) > manHattanDistance(k) {
				minDistance = k
			}
		}
	}
	fmt.Println(manHattanDistance(minDistance))
