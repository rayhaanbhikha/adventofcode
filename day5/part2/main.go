...
func runProgram(program []int, input int) int {
	...
	for index := 0; index < len(p.program); {
		...
		if newIndex, change := computeIntcode(&p, input, opCode, paramModes, intcode, &output); change {
			index = newIndex
		} else {
			index = newIncrement
		}
...
func computeIntcode(p *Program, input, opcode int, paramModes, intcode []int, output *int) (int, bool) {
